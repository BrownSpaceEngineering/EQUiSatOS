...
uint8_t initialize_master(struct spi_module *spi_master_instance, uint32_t baudrate){
	...
	config_spi_master.pinmux_pad0 = MRAM_SPI_MISO;
	config_spi_master.pinmux_pad1 = PINMUX_UNUSED;
	config_spi_master.pinmux_pad2 = MRAM_SPI_MOSI;
	config_spi_master.pinmux_pad3 = MRAM_SPI_SCK;
...
status_code_genare_t read_bytes(struct spi_module *spi_master_instance, struct spi_slave_inst *slave, uint8_t *data, int num_bytes, uint16_t address){
	...
	s = spi_transceive_buffer_wait(spi_master_instance, read_control, control_rx_temp, NUM_CONTROL_BYTES + 1);
	if (!status_ok(s)) return s;
