...
void watch_test(void) {
	...
	check_out_task(ANTENNA_DEPLOY_SHIFT);
	
	check_in_task(TRANSMIT_SHIFT);
	running_task(TRANSMIT_SHIFT);
	check_out_task(TRANSMIT_SHIFT);
	...
	check_out_task(BATTERY_CHARGING_SHIFT);
