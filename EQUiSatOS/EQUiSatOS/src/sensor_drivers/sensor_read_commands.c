...
static void srcReadAdc(uint8_t* dest, uint8_t pin, uint8_t eloc, bool priority) {
...
void read_lion_volts_batch(lion_volts_batch batch) {
	...
	log_if_error(ELOC_L1_REF, sc, true);
	sc = read_adc(adc_instance1, &buf);
	log_if_error(ELOC_L1_REF, sc, true);
	batch[0] = buf;
	sc = configure_adc(&adc_instance2, P_AI_L2_REF);
	log_if_error(ELOC_L2_REF, sc, true);
	sc = read_adc(adc_instance2, &buf);
	log_if_error(ELOC_L2_REF, sc, true);
	batch[1] = buf;
}
...
void read_lifepo_current_batch(lifepo_current_batch batch) {
	...
	enum status_code sc = configure_adc(&adc_instance1, P_AI_LFB1SNS);
	sc = read_adc(adc_instance1, &buf);
	batch[0] = buf;
	sc = configure_adc(&adc_instance2, P_AI_LFB1OSNS);
	sc = read_adc(adc_instance2, &buf);
	batch[1] = buf;
	sc = configure_adc(&adc_instance3, P_AI_LFB2SNS);
	sc = read_adc(adc_instance3, &buf);
	batch[2] = buf;
	sc = configure_adc(&adc_instance4, P_AI_LFB2OSNS);
	sc = read_adc(adc_instance4, &buf);
	batch[3] = buf;
}
