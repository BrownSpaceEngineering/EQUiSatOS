...
// size of LEDFlash binary in bytes
#define BINSIZE 1848 
// address at which binary is stored in mram
#define MRAM_APP_ADDRESS 0x0400
...
int main(void)
{
	...
	mram_read_bytes(&spi_master_instance, &slave1, APP_START_ADDRESS, BINSIZE, MRAM_APP_ADDRESS);
	mram_read_bytes(&spi_master_instance, &slave2, APP_START_ADDRESS, BINSIZE, MRAM_APP_ADDRESS);
